...
export class AuthorsComponent implements OnInit {
  ...
  constructor(
    ...
    effect(() => {
      ...
      this.loading.set(this.authorsStore.loading());
      this.error.set(this.authorsStore.error());
    });
  }
  ...
  saveAuthor(): void {
    ...
    const exists = this.authors().some(a => a.name.toLowerCase() === name.toLowerCase());
    if (exists && !this.editingAuthor()) {
      this.error.set('Já existe um autor com este nome.');
      return;
    }
    this.loading.set(true);
    this.error.set(null);
    ...
    const operation = editing
      ? this.authorsStore.update(editing.id, dto)
      : this.authorsStore.create(dto);
    ...
    operation.subscribe({
      ...
      error: (err) => {
        ...
        if (err.status === 0) {
          ...
          errorMessage = 'Já existe um autor com este nome.';
        } else if (err.status === 500) {
          errorMessage = 'Erro interno no servidor ao salvar autor.';
          if (typeof err.error === 'string') {
  ...
  confirmDelete(): void {
    ...
    this.authorsStore.delete(author.id).subscribe({
      next: () => {
